...
class smartcity_recent_post_widget extends WP_Widget {
	/* Widget setup */
	public function __construct() {
		$widget_ops = array(
			'classname'   => 'recent-post-widget',
			'description' => esc_html__( 'A widget show post', 'smartcity' ),
		);
	...
	function form( $instance ) {
		...
		$defaults = array(
			'title' => esc_html__('Recent Post', 'smartcity'),
			'order' => 'DESC'
		);
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>">
				<?php esc_attr_e( 'Select Categories:', 'smartcity' ); ?>
            </label>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>"
                ...
                <option value="0" <?php echo( in_array( 0, $select_cat ) ? 'selected="selected"' : '' ); ?>>
					<?php esc_html_e( 'All Category', 'smartcity' ); ?>
                </option>
        ...
        <p>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'order_by' ) ); ?>"
                ...
                <option value="ID" <?php echo ( $order_by == 'ID' ) ? 'selected' : ''; ?>>
					<?php esc_html_e( 'ID', 'smartcity' ); ?>
                </option>

                <option value="date" <?php echo ( $order_by == 'date' ) ? 'selected' : ''; ?>>
					<?php esc_html_e( 'Date', 'smartcity' ); ?>
                </option>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>">
				<?php esc_html_e( 'Number of posts to show:', 'smartcity' ); ?>
            </label>
...
// Register widget
function smartcity_recent_post_widget_register() {
	register_widget( 'smartcity_recent_post_widget' );
}

add_action( 'widgets_init', 'smartcity_recent_post_widget_register' );